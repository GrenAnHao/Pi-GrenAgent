...
- 原型：`.superpowers/brainstorm/grenagent-ui/content/*.html`（b-realistic / module-knowledge / modules-mem-review-create / modules-conn-settings）
...
左侧新增**模块导航栏**（窄图标栏，约 56–60px），切换中央主视图：

| 图标 | 模块 | 承载 |
|---|---|---|
| 💬 | 对话 | 聊天 + web-fetch / multi-agent / tts 工具卡片 + 知识/记忆自动注入提示 |
| 📚 | 知识库 | knowledge-rag 管理 |
| 🧠 | 记忆 | long-term-memory 管理（项目 + 全局） |
| 🔍 | 审查 | code-review |
| 🎨 | 创作 | image-gen（未来可扩展 video/音频） |
| 🔌 | 连接 | im-gateway 网关与平台接入 |
| ⚙️ | 设置 | 各 extension 的 key / 开关集中管理 |

**取舍**：web-fetch / multi-agent / tts 不单独成模块 —— 它们是「即时动作」而非「需要管理的数据」，归入对话模块作为工具。

**管理视图统一范式**：📚🧠🔍🎨 四个管理模块共用「**顶部状态+操作 / 左列表 / 右详情**」骨架，只换内容，降低实现与认知成本。
...
- **会话列表**：沿用现有 `Sidebar`（按项目分组、运行中绿点）。注意：模块导航栏在会话列表更左侧，二者并存。
- **消息流**：现有 `MessageList` + `ToolExecution` 卡片渲染，新增对新工具的专用渲染（§6）。
- **自动注入提示**：消息流顶部/相应轮次插入轻量提示条「🧠 已注入 N 条记忆 · 📚 M 条知识片段」，来源是 `before_agent_start` 注入的 `customType` 消息（knowledge-rag / long-term-memory）。
- **输入区快捷**：`ActionBar` 增加 `+知识库`（把当前文件/选区 kb_add）、`/生图`、`🔊朗读`，与现有 模型/thinking/上传/compact 并列。
...
新工具的 `ToolExecution` 专用渲染（沿用现有工具卡片机制，按 `toolName` 分派）：

- `kb_search`：命中片段列表（source + score + 文本）。
- `kb_add`：索引结果（source + chunks 数 + embedded/keyword）。
- `memory_save` / `memory_recall`：保存确认 / 召回条目。
- `generate_image`：图片缩略图 + prompt + 保存路径，点击放大。
- `spawn_agent`：子 agent 树（任务 + 状态 + 输出折叠）。
- `fetch_url`：来源 URL + 抓取标题 + markdown 摘要。
- `speak`：音频播放控件（路径 + ▶）。

自动注入提示：knowledge-rag / long-term-memory 的 `before_agent_start` 注入消息（`customType: "knowledge-rag" | "long-term-memory"`，`display:true`）在前端识别为「注入提示条」而非普通消息渲染。

## 7. 前端组件设计（在 `tauri-agent/src/` 内）

遵循现有 `features/<domain>` 结构，新增：

- `features/layout/ModuleRail.tsx` — 左侧模块导航栏（图标 + 高亮 + 切换）。
...
- **管理面板读写数据**：两条可选路径，二选一（实现时定，见非目标）：
  1. **复用工具/命令**：面板操作 = 让 sidecar 执行对应工具（如知识库面板「添加」= 调 `kb_add`），结果回流。改动小、复用 RPC。
...
1. **骨架**：`ModuleRail` + 模块容器 + `moduleStore`，空面板占位（验证导航）。
